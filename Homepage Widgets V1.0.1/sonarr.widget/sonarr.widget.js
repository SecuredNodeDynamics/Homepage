/* =====================================================
   SONARR WIDGET
   Group name: ARR — SONARR
===================================================== */
(function () {

  const SONARR_CONFIG = {
    groupName: "ARR — SONARR",
    servers: [
      { label: "Sonarr", url: "http://YOUR_LOCAL_IP:PORT", fallbackUrl: "https://YOUR_TUNNEL_URL", activeUrl: null, key: "YOUR_API_KEY_HERE", href: "http://YOUR_BASE_URL" },
    ],
    pollMs: 120 * 1000,
  };
  ...
  async function fetchStats(server) {
    const [series, queue, wanted] = await Promise.all([
      fetchArr(server, `/api/v3/series?apikey=${server.key}`),
      fetchArr(server, `/api/v3/queue?apikey=${server.key}`),
      fetchArr(server, `/api/v3/wanted/missing?apikey=${server.key}&pageSize=1`),
    ]);
    const episodes = Array.isArray(series)
      ? series.reduce((acc, s) => acc + (s.statistics?.episodeFileCount ?? 0), 0) : 0;
    return {
      series: Array.isArray(series) ? series.length : 0,
      monitored: Array.isArray(series) ? series.filter(s => s.monitored).length : 0,
      episodes,
  ...
  function buildStatsHtml(stats) {
    return `
      <div class="arr-stat-grid">
        <div class="arr-stat">
          <div class="arr-stat-value">${fmtNum(stats.series)}</div>
          <div class="arr-stat-label">Series</div>
        </div>
        <div class="arr-stat">
          <div class="arr-stat-value">${fmtNum(stats.episodes)}</div>
          <div class="arr-stat-label">Episodes</div>
        </div>
  ...
  function buildShell(activeIdx, statsHtml, loading) {
    const tabs = SONARR_CONFIG.servers.map((s, i) => `
      <button class="arr-tab sonarr ${i === activeIdx ? "arr-tab--active" : ""}" data-idx="${i}">
        ${escH(s.label)}
      </button>`).join("");

    const activeServer = SONARR_CONFIG.servers[activeIdx];
  ...
  async function render(host, activeIdx) {
    ...
    try {
      const stats = await fetchStats(SONARR_CONFIG.servers[activeIdx]);
      host.innerHTML = buildShell(activeIdx, buildStatsHtml(stats), false);
    } catch (err) {
      console.error("[Sonarr Widget]", err);
  ...
  function init() {
    const start = () => setTimeout(() => {
      const group = findGroup(SONARR_CONFIG.groupName);
      if (!group) return;
      _host = ensureHost(group, "arr-sonarr-host");
      render(_host, 0);
      setInterval(() => {
        if (document.hidden) return;
        const activeIdx = parseInt(_host.querySelector(".arr-tab--active")?.dataset.idx ?? 0);
        render(_host, activeIdx);
      }, SONARR_CONFIG.pollMs);
    }, 1200);
    ...
    new MutationObserver(() => {
      ...
      setTimeout(() => {
        pending = false;
        if (!document.querySelector(".arr-sonarr-host .arr-shell")) {
          const group = findGroup(SONARR_CONFIG.groupName);
          if (!group) return;
          _host = ensureHost(group, "arr-sonarr-host");
          render(_host, 0);
