...
(function () {
    ...
    function itemHref(item) {
      ...
      if (!id) return base;
      const type = item?.mediaType || "book";
      if (type === "podcast") return `${base}/podcast/${id}`;
      return `${base}/audiobook/${id}`;
    }
    ...
    async function renderLibrary() {
      ...
      statsResults.forEach((res, i) => {
        ...
        totalTracks += s.numAudioTracks || s.numAudioTrack || 0;
        libCards.push(`
          <div class="abs-lib-card">
      ...
      const recentSections = await Promise.allSettled(
        libs.map(lib => apiFetch(`/api/libraries/${lib.id}/personalized`))
      );
      ...
      return `
        ...
        ${libCards.length ? `
          ...
          <div class="abs-lib-grid">${libCards.join("")}</div>` : ""}
        ${recentItems.length ? `
        ...
        <div class="abs-shell">
          <div class="abs-header">
            ...
            <div class="abs-header-right">
              ...
              <a class="abs-open-btn" href="${escH(openUrl())}" target="_blank" rel="noopener noreferrer">
                Open
